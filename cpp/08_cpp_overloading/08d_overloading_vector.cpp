...
// Here is a Vector3 struct we can use to represent
// a vector in 3-dimentional space, with separate x, y, z
// variables that can be directly accessed via dot notation:
struct Vector3 {
  float x, y, z;
};
...
int main(void) {
  ...
  // Unlike arrays, we can create deep copies of structs
  // using the assignment operator (however, if the
  // struct contains an array as an member variable,
...
float vecNorm(Vector3 a) { return sqrt(vecDot(a, a)); }
