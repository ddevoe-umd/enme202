...
// operator definition.
//
// We cannot overload operators with built-in types only.
// At least one of the operator arguments must be a custom
// data type (e.g. a struct or class).
//
// Almost all C++ operators (arithmetic, relationl, logical,
// bitwise, assignment, etc.) can be overloaded, with only
// a few exceptions such as :: (scope resolution) and . (dot).
