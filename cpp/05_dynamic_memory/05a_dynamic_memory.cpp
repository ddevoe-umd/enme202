...
int main(void) {
  ...
  // The dereference operator can be used to both get and set
  ...
  *xPointer = 4;
  ...
  //
  // In reality, there is no real difference in the use of "*".  
  // The C++ compiler doesn't care if we write:
  int* nPointer1;
  // or
  int *nPointer2;
  // The first is more typical in C++ code, but either is valid.
  //
  // In practice, the second version is arguably better. Consider
  // the following line:
  int* x1, x2;
  // This line does NOT create 2 pointers!  It creates one pointer
  // to an int (*x1) and one int (x2).  To create two pointers, you
  // would instead need to do the following:
  int *y1, *y2;
  ...
  cout << "&A[0]+1 = Address of A[1]: " << &A[0]+1 << "\n\n";
  // What about &A+1 ?
  cout << "&A+1 = Address after end of full array: " << &A+1 << "\n\n";

  // So &A+1 goes to the next memory location after
  // the END of the array, while A+1 yields the address of A[1].  

  // Just a reminder about dereferencing:
  cout << "*A+1 = Value of A[0] + 1: " << *A+1 << endl;
  cout << "*(A+1) = Value of A[1]: " << *(A+1) << endl;

  // Above we saw that for an array, we cannot directly learn the 
  // address where the value A is stored using &A. Instead, A and &A
