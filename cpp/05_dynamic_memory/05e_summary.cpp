...
int main(void) {
  ...
  int* x = new int;   // allocate heap memory for an integer and 
                      // create pointer x pointing to base address
  cout << "\nScalar pointer x:" << endl << "---------------" << endl;
  *x = 999;           // dereference to place value into allocated memory
  cout << "x:     " << x << endl;     // pointer (value of x = address x points to)
  cout << "*x:    " << *x << endl;    // dereferenced pointer (value in memory x points to)
  cout << "&x:    " << &x << endl;    // referenced pointer (address where x lives)
  cout << "*(&x): " << *(&x) << "\n\n"; // value in address of x (same as just plain x)
  ...
  cout << "*(&y): " << *(&y) << endl; // same as &y or &y[0] -- NOT the value of y[0]
  cout << "**(&y): " << **(&y) << "\n\n"; // value of y[0]
  ...
  cout << "*(&z): " << *(&z) << endl; // value in address of z (=z itself)
  cout << "**(&z): " << **(&z) << "\n\n"; // value of z[0]
