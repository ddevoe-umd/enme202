...
# Exception object (a data element created when an error occurs)
...
try:
	...
	print(f'there are only {len(y)} elements in the list')
except TypeError:
...
try:
	...
	print('This block only runs if there was an error in the try block')
	print(f'here is the error: {e}')
else:
    print('Run this block only if no exception occurred in the try block')
finally:
    print('Perform clean-up operations regadless of whether there was')
    print('an error, for example close files, reset global variables, etc.')
