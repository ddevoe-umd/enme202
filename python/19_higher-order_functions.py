...
# returns a value to the calling code.  Nothing new here...
#
# However, if the outer function returns the inner function itself
# this is termed a _closure_ since it "closes over" the environment in which
...
#
# Closure enables the creation of "function factories" that can generate
# new functions, as seen in the following example:
...
# What if we want the wrapper to work with an arbitrary number of arguments?
# Remember the following syntax for packing arguments with *args:

def decorator_with_changing_arguments(func):
    def wrapper(*args):
        for val in args:
            print(val)
        return func(*args)
...
@decorator_with_changing_arguments
def add1(a, b, c):
    print(f'sum = {a+b+c}')
...
# Note that we also need to use **kwargs when passing dictionaries as arguments.
